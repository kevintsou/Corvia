...
def _header(template_id: str, reasons: Iterable[str]) -> str:
    ...
    # Stamp the live tool version as the config-version marker. Without this a
    # generated corvia.toml carried no marker and failed the version check on
    # its very next run (see config._check_config_version). Deriving it from
    # __version__ here also makes __init__.py the single source of truth -- a
    # version bump no longer needs the marker hand-edited across template files.
