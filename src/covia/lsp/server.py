"""COVIA Language Server (LSP).

Run with:
    covia-lsp --stdio
    covia-lsp --tcp --host 127.0.0.1 --port 9999

The server analyzes a document on open and on save, then publishes
diagnostics. It uses COVIA's incremental cache so repeated edits to a
single file in a workspace stay fast.

This module only imports pygls when invoked, so importing
`covia.lsp.server` without pygls installed will fail with a friendly
...
def _import_language_server():
    try:
        ...
        raise SystemExit(
            "lsprotocol is not installed. Install with `pip install 'covia[lsp]'`."
        ) from e
    try:
        ...
        try:
            ...
            raise SystemExit(
                "pygls is not installed. Install with `pip install 'covia[lsp]'`."
            ) from e
...
def create_server():
    ...
    server = LanguageServer("covia-lsp", "0.1.0")
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="covia-lsp", description="COVIA Language Server")
